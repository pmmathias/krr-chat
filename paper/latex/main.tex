...
Table~\ref{tab:benchmarks} reports wall-clock time and peak memory for the two solvers on synthetic SPD problems with structure $A = Z^\top Z + \lambda I$, $\lambda = 10^{-6}$, $V = 500$, two runs per configuration (median reported). All measurements are CPU-only on Apple Silicon.
...
\bottomrule
\end{tabular}
\caption{Solver comparison at varying RFF dimension $D$. CG iteration counts grow sublinearly with $D$, consistent with $O(\sqrt{\kappa})$ scaling.}
\label{tab:benchmarks}
\end{table}

\paragraph{Honest assessment.} On CPU at Kalle's current scale, direct solve is faster in wall-clock time because LAPACK's Gaussian elimination is extraordinarily well-tuned and $O(D^3)$ remains tractable through $D \sim 10^4$. The advantage of Block-PCG becomes decisive under three conditions, each of which applies to different deployment scenarios:
\begin{enumerate}
  \setlength\itemsep{0pt}
  \item \textbf{Very large $D$} ($D \gtrsim 10^4$): memory dominates, and the $O(D^2)$ storage of the direct system exceeds device capacity.
  \item \textbf{GPU acceleration}: matrix-vector products on GPU are $\sim 10$--$100\times$ faster than on CPU, while \texttt{solve}-style routines parallelize poorly; the crossover shifts to $D \sim 10^3$.
  \item \textbf{Approximate solutions acceptable}: PCG produces useful $W$ after a handful of iterations, enabling early-stopping as implicit regularization \citep{yao2007early}.
\end{enumerate}
Kalle v2 ships with both solvers (\texttt{src/build\_v2.py --solver \{direct,cg\}}), enabling future scale-up paths without architectural changes.
