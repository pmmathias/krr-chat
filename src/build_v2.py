"""
Build kalle-chat.html — the complete KRR language model (v2 with pluggable solvers).

This is the v2 build pipeline. Identical to build.py except for step 7
(linear system solve), which is now pluggable via --solver:

  --solver=direct  → numpy.linalg.solve (Kalle v1 baseline, O(D^3) compute, O(D^2) memory)
  --solver=cg      → Block Preconditioned Conjugate Gradient (O(sqrt(κ)) iters, O(D·V) memory)
  --solver=power   → Power iteration on absorber-stochasticized matrix (didactic, slow)

The solvers and their derivation are documented in:
  paper/theory/absorber_stochastisierung.md

Hyperparameters (unchanged from v1):
  CTX=24, EMB_DIM=32, FEAT=768, D=6144, σ=1.5, λ=1e-6

Usage:
  python3 src/build_v2.py --solver=cg
  python3 src/build_v2.py --solver=direct --output=kalle-chat-direct.html
  python3 src/build_v2.py --solver=cg --cg-tol=1e-7 --cg-maxiter=300
"""
import re, json, base64, gzip, math, time, os, sys, argparse
import numpy as np
from gensim.models import Word2Vec

# Import solvers from sibling module
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from solvers import solve as krr_solve

# --- CLI args with defaults pointing to repo structure ---
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
parser = argparse.ArgumentParser(description='Build KRR Chat HTML (v2 with pluggable solvers)')
...
parser.add_argument('--output', default=os.path.join(ROOT, 'kalle-chat-v2.html'))
parser.add_argument('--solver', choices=['direct', 'cg', 'power'], default='cg',
                    help='Linear solver for KRR (default: cg)')
...
# Solve via pluggable solver (T029)
ZtZ += LAMBDA * np.eye(D)
print(f"  solving linear system via solver={args.solver}...")
t0 = time.time()
...
if args.solver == 'cg':
    solver_kwargs = {
        'tol': args.cg_tol,
        'max_iter': args.cg_maxiter,
        'preconditioner': args.cg_precond,
        'verbose': True,
    }
elif args.solver == 'power':
    # power needs lambda separately (it operates on un-regularized matrix)
    solver_kwargs = {
        'lambda_reg': LAMBDA,
        'tol': args.cg_tol,
        'max_iter': args.cg_maxiter * 10,  # power needs many more iterations
        'verbose': True,
    }

W_f64, solver_info = krr_solve(ZtZ, ZtY, solver=args.solver, **solver_kwargs)
W = W_f64.astype(np.float32)
solve_time = time.time() - t0
print(f"  solved in {solve_time:.1f}s, W: {W.shape}")
...
solver_label = {'direct': 'Direct Solve', 'cg': 'Block-PCG', 'power': 'Power Iteration'}[args.solver]
new_header = f'<p>KRR Buddy-Chat Kalle v2. {V} Woerter, {len(pairs)} Dialoge. Top-1: {top1*100:.1f}%. Solver: {solver_label}. Kein neuronales Netz.</p>'
