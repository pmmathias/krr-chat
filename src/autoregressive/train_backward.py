#!/usr/bin/env python3
"""
Phase 2: Train a BACKWARD (right-to-left) KRR language model.

Identical to train_gpu.py but trains on the REVERSED token stream.
This gives us P(word | right_context) — the other half of bidirectional prediction.

Combined with the forward model:
  P_bidi(word | left_ctx, right_ctx) ∝ P_fwd(word | left_ctx) × P_bwd(word | right_ctx)

Usage:
  python train_backward.py --D 12288 --corpus corpus_15m.txt
  # or for the 3M corpus:
  python train_backward.py --D 12288 --corpus corpus_large.txt --tokenizer bpe_large.json
"""
...
def parse_args():
    p = argparse.ArgumentParser(description='Backward KRR-LM')
    ...
    p.add_argument('--output', default='data/autoregressive/model_backward.pkl')
...
def block_cg_gpu(A, B, tol=1e-5, max_iter=500, verbose=False):
    ...
    for it in range(max_iter):
        ...
        if verbose and it % 100 == 0: print(f"    CG {it+1}: {rel_res:.2e}")
        if rel_res < tol: return X, {'iterations': it+1, 'converged': True}
...
def gpu_kmeans(features, K, n_iters=50, seed=42):
    ...
    for it in range(n_iters):
        dists = torch.cdist(features, centers); assignments = dists.argmin(dim=1)
...
def main():
    ...
    print(f"=== BACKWARD KRR-LM (right-to-left) ===")
    print(f"D={args.D}, MoE K={args.experts}")
    ...
    # >>> KEY DIFFERENCE: REVERSE the token stream <<<
    print("Reversing token stream for backward model...")
    all_ids_reversed = all_ids[::-1].copy()
    print(f"  Original first 10: {all_ids[:10]}")
    print(f"  Reversed first 10: {all_ids_reversed[:10]}")

    n_val = int(len(all_ids_reversed) * args.val_frac)
    train_ids = all_ids_reversed[:len(all_ids_reversed) - n_val]
    val_ids = all_ids_reversed[len(all_ids_reversed) - n_val:]
    N = len(train_ids) - args.ctx
    print(f"Corpus: {len(all_ids_reversed):,} tokens (reversed), N={N:,}")

    CTX, EMB, DK, DV, H, DFF, D = args.ctx, args.emb_dim, args.dk, args.dv, args.heads, args.dff, args.D
    FEAT = 2 * EMB; K = args.experts

    # ---- Word2Vec on REVERSED stream ----
    print("Word2Vec on reversed stream...")
    from gensim.models import Word2Vec
    str_toks = [str(i) for i in train_ids]
    sents = [str_toks[i:i+100] for i in range(0, len(str_toks), 100)]
    w2v = Word2Vec(sents, vector_size=EMB, window=8, min_count=1, workers=4, sg=0, epochs=args.w2v_epochs, seed=args.seed+999)
    ...
    # Use DIFFERENT random seed for backward attention (independent from forward)
    si = 1.0 / math.sqrt(EMB)
    g = torch.Generator(device='cpu').manual_seed(args.seed + 7777)
    attn = {
    ...
    g_rff = torch.Generator(device='cpu').manual_seed(args.seed + 8888)
    ...
    # ---- Encode + MoE training (identical to forward, but on reversed data) ----
    print(f"\nEncoding {N:,} reversed contexts...")
    ...
    centers, _ = gpu_kmeans(all_feats[sub_idx].to(dev), K, n_iters=args.kmeans_iters)
    ...
    for k in range(K):
        ...
        if n_k < 100: expert_W.append(None); continue
        print(f"  Expert {k}: {n_k:,}", end="", flush=True)
        t0 = time.time()
        ...
        expert_W.append(W_k); del ZtZ_k, ZtY_k

    # Global fallback
    print("  Global W...", end="", flush=True)
    t0 = time.time()
    ZtZ_g = torch.zeros(D, D, dtype=torch.float64, device=dev)
    ZtY_g = torch.zeros(D, V, dtype=torch.float64, device=dev)
    for start in range(0, N, BS):
        end = min(start + BS, N)
        feat_batch = all_feats[start:end].to(dev); tgt_batch = all_targets[start:end]
        with torch.no_grad():
            z = rff_scale * torch.cos(feat_batch @ omega_t + bias_t.unsqueeze(0))
    ...
    # Eval on reversed val set
    print("\nEval (backward, on reversed val)...")
    ...
    for i in sample:
        ...
        E = emb_t[ctx].unsqueeze(0); X = E + pe_t[-CTX:].unsqueeze(0)
        Q_ = (X @ attn['W_Q']).view(1, CTX, H, DK).permute(0, 2, 1, 3)
        K_ = (X @ attn['W_K']).view(1, CTX, H, DK).permute(0, 2, 3, 1)
        Vv_ = (X @ attn['W_V']).view(1, CTX, H, DV).permute(0, 2, 1, 3)
        sc = (Q_ @ K_) / math.sqrt(DK)
        sc.masked_fill_(causal_mask.unsqueeze(0).unsqueeze(0), float('-inf'))
        ao = (torch.softmax(sc, -1) @ Vv_).permute(0, 2, 1, 3).reshape(1, CTX, H*DV) @ attn['W_O']
        ...
        with torch.no_grad():
            ...
            eidx = d.argmin().item()
            scores = (z @ expert_W[eidx]).squeeze()
        tgt = val_ids[i + CTX]
        if scores.argmax().item() == tgt: top1 += 1
        _, t5 = scores.topk(5)
        if tgt in t5.cpu().numpy(): top5 += 1
    n = len(sample)
    print(f"  Backward Val Top-1: {top1/n*100:.1f}%, Top-5: {top5/n*100:.1f}%")
    ...
    with open(args.output, 'wb') as f:
        pickle.dump({
            ...
            'emb': emb_t.cpu().numpy(), 'pe': pe_t.cpu().numpy(),
    ...
    print(f"\n{'='*60}")
    print(f"BACKWARD MODEL DONE (MoE K={K})")
